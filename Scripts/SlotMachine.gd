...
func _on_enter_interaction_area(body: Node2D) -> void:
	if not body.is_in_group("Player") or HUD.in_critical_debt: return
	Overlay.overlay_alpha(overlay_show_alpha, overlay_tween_duration)
	Text.show_text()

func _on_exit_interaction_area(body: Node2D) -> void:
	if not body.is_in_group("Player") or HUD.in_critical_debt: return
	Overlay.overlay_alpha(0, overlay_tween_duration)
	Text.hide_text()
...
func interact_with_slot_machine():
	if HUD.in_critical_debt: return
	sprite.play("button")
